...
class ViewPaperFragment : Fragment() {
    ...
    private fun init() {
        val listFragment = arrayListOf(ChatsFragment(), SearchFragment(), GroupsFragment(), ProfileFragment())
        ...
        val adapter = ViewPager2Adapter(listFragment, childFragmentManager, lifecycle)
        with(binding) {
            viewPager.adapter = adapter
            rootView = mainLayout.rootView
            profileImageHome.setImageBitmap(convert(preferenceManager.getString(KEY_IMAGE)!!))
            usernameHome.text = preferenceManager.getString(KEY_NAME)
        }
    }
    ...
    private fun onTabChats() {
        with(binding) {
            ...
            setIconColor(requireActivity(), R.drawable.ic_logout, buttonLogout, R.color.chats_100)
            toolbarHome.setBackgroundColor(getColorResource(requireActivity(), R.color.chats_500))
            setBottomNavigationBackgroundColor(getColorResource(requireActivity(), R.color.chats_200))
        }
    }
    ...
    private fun onTabSearch() {
        with(binding) {
            ...
            setIconColor(requireActivity(), R.drawable.ic_logout, buttonLogout, R.color.search_100)
            toolbarHome.setBackgroundColor(getColorResource(requireActivity(), R.color.search_500))
            setBottomNavigationBackgroundColor(getColorResource(requireActivity(), R.color.search_200))
        }
    }
    ...
    private fun onTabGroups() {
        with(binding) {
            ...
            setStatusBarColor(requireActivity(), getColorResource(requireActivity(), R.color.groups_500))
            setIconColor(requireActivity(), R.drawable.ic_logout, buttonLogout, R.color.groups_100)
            toolbarHome.setBackgroundColor(getColorResource(requireActivity(), R.color.groups_500))
            setBottomNavigationBackgroundColor(getColorResource(requireActivity(), R.color.groups_200))
        }
    }
    ...
    private fun onTabProfile() {
        with(binding) {
            ...
            setStatusBarColor(requireActivity(), getColorResource(requireActivity(), R.color.profiles_500))
            setIconColor(requireActivity(), R.drawable.ic_logout, buttonLogout, R.color.profiles_100)
            toolbarHome.setBackgroundColor(getColorResource(requireActivity(), R.color.profiles_500))
            setBottomNavigationBackgroundColor(getColorResource(requireActivity(), R.color.profiles_200))
        }
    }

    private fun setBottomNavigationBackgroundColor(@ColorInt color: Int) {
        when (val background = binding.bottomNavigation.background!!) {
    ...
    private fun getUserAuthToSignOut() {
        databaseViewModel.logout(preferenceManager.getString(KEY_USER_ID)!!)
            ...
            if (it) {
                loginViewModel.firebaseAuth.observe(viewLifecycleOwner) { auth ->
                    ...
                    FirebaseAuth.getInstance().currentUser?.let { user ->
                        databaseViewModel.updateStatus(user.uid, false)
                    }
